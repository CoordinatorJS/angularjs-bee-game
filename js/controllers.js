...
app.controller("mainCtrl", function($scope, $mdToast) {
  ...
  $scope.randomBee =
    $scope.allBees[Math.floor(Math.random() * $scope.allBees.length)];

  $scope.hitBee = function() {
    console.log($scope.allBees);
    console.log($scope.randomBee);

    var pos = $scope.allBees
      .map(function(e) {
        return e.name;
      })
      .indexOf($scope.randomBee.name);
    if (pos > -1) {
      $scope.allBees[pos].life -= $scope.randomBee.hit;
      if ($scope.allBees[pos].life <= 0) {
        $scope.allBees[pos].life = 0;
        if ($scope.allBees[pos].name === "Queen Bee") {
          // If Queen Bee is dead, Game Over
        ...
        console.log(
          $scope.randomBee.name + " life = " + $scope.allBees[pos].life
        );
        toast(
          $scope.randomBee.name + " hit by " + $scope.randomBee.hit + " points"
        );
      }
    }
    $scope.randomBee =
  ...
  function toast(text) {
    //Create toast on the top-right hand corner
    $mdToast.show(
      $mdToast
        .simple()
        .content(text)
